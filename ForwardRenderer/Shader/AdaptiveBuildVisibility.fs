...
layout(binding = 0, rg32f) uniform image3D tex_visz;

layout(binding = 5, std430) buffer ubo_atomics
{
	uint b_screenWidth;
	uint b_lockId; // this will be incremented for each fragment
	uint b_atomics[];
};
...
void lock()
{
	uint index = uint(gl_FragCoord.y) * b_screenWidth + uint(gl_FragCoord.x);
	
	// make sure we dont get a 0 for lock id
	uint lockId = 0u;
	while(lockId == 0u)
		lockId = atomicAdd(b_lockId, 1);
	
	// try to set buffer to lockId
	uint res = 0u;
	
	while(res != lockId)
	{
		atomicCompSwap(b_atomics[index], 0u, lockId);
		// what is the new value?
		res = atomicAdd(b_atomics[index], 0u);
	}
